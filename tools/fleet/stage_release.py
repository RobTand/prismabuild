...
def sweep_dead_owner_fragments(
    ...
    installed.  This pass deletes none of them.  Their deletion authority is the same death proof, taken
    again under the consumer's lock by :func:`sweep`, and pressure only
    decides when; ``evict`` rechecks co-owners, claims, pins and handoffs
    under its own locks.  A discovery that could not complete marks the
    collection unknown rather than empty.
                        ...
                        with queue.mover_transition_lock(mover):
                            ...
                            if material_present:
                                ...
                                if (coherent and uncharged is not None
                                        and mover not in held_by_tier[tier]):
                                    # #1061: whole, current and holding no
...
def sweep(queue: pool.PoolQueue, *, stage_roots: dict[str, str],
    ...
    **``index`` is the tier loop's census, kept from one cycle to the next
    ...
    swept.extend(sweep_dead_owner_fragments(
        ...
        index=index, uncharged=uncharged))
