...
- PRG-03 waits are typed: fit-lack waits (`residency_lead_not_resident`
  ...
  phases `copy` and `warm`, reports its landed bytes on the heartbeat, and
  ends `no_progress` when a copy lands nothing for
  `ceil(unit / rate + 2 * HEARTBEAT_S)`, where the unit is the chunk's 16
  largest entries. Its ending names the bytes landed and the range, and it
  follows the retry and `failed` path, so a consumer's exemption does not
  outlive a stalled copy. The landing record prices a claimed copy from
  its last report (`basis: reported`). A mover with no measured rate is
  sealed as before (`basis: unmeasured`).
