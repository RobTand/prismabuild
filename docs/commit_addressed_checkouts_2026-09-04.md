...
An action carries `checkout_root`, an absolute path (`pool.py:558-608`,
...
(`core.py:3595-3611`), which is what allows two boxes to hold two claims
without contending.
...
this fleet already relies on for token minting (`pool.py:288-291`). That it
...
between sealing and running (`tools/fleet/pool_reset.py:12-16`). Requiring a
...
edit must move the action key (`pbrun.py:97-110`). Include the same
...
3. Run exactly as today: `worker_argv` gets `--checkout-root <that path>`
   (`pool.py:1180-1190`), and everything downstream is unchanged.
4. The closure check keeps its teeth. The materialiser writes the stamp by
   ...
   `pbrun` does at submit; `core.verify_code_closure` (`core.py:1135-1147`)
...
* **`TRITON_CACHE_DIR` stays `/home/rob/.triton-cache`** — a local path per
  box, same string, different disk (`pbrun.py:406-414`).
