...
| `unreadable` | Not a filed status. `pbstatus` prints this row for a record it could not read, so a truncated, oversized, or inaccessible newest record does not print the empty table a fleet that filed nothing prints. The note column names the reason and the path: `permission denied`, the OS error's own text, `not valid JSON`, `not a JSON object`, or the 8 MiB reader limit. No other column carries a value, because every other column is inside the file nobody could read. The row is placed by the file's modification time. |
...
- every empty `residency/<consumer>/` directory whose consumer meets the same
  rule, with its map `residency/<consumer>.map.json`;
- every landing record `residency/<consumer>.landing.json` whose consumer
  meets the same rule, whether or not its fragments remain (#989).
...
A namespace or landing record is removed under its consumer's transition lock,
taken without waiting, after re-reading that the consumer was not queued
again. Each run writes a JSON receipt to `<queue>/gc-receipts/`, or to
...
`power_reference_w` with scope `soc_tdp` for provenance, and reaches a fraction
only under that scope, on a device nothing better is known about. Receipts and
metrics divide by the same reference admission does, not by the SoC envelope:
`adaptive_gpu.reporting_power_reference` defers to the admission derivation,
which returns the highest GPU-only draw the host has sampled from that device,
floored by the declared per-device capacity fact
`adaptive_gpu.DECLARED_GPU_POWER_REFERENCE_W` (#806). The scope travels beside
every published ratio, so a fraction is never read against a reference nobody
named. Neither number is a saturation target.
