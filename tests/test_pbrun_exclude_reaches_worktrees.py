...
the stamp and result-log prefixes to git's local exclude file so they cannot
make a clean tree look dirty to anything else.  It computed that file as
...
def test_a_linked_worktree_is_taught_to_ignore_the_stamp(tmp_path: Path) -> None:
    ...
    stamp = f"{pbrun.STAMP_PREFIX}abc.json"
    ...
    assert _ignored(tree, f"{pbrun.RESULT_PREFIX}abc.txt")
