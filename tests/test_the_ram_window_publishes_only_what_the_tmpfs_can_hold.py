...
def test_the_first_phase_is_promoted_when_its_stage_range_has_landed(
        tmp_path: Path) -> None:
    # The corrected joint-fit policy (#745) admits a newcomer on its current
    # plus its protected next, so the tmpfs must hold both 2 GiB phases.
...
def test_nothing_is_promoted_against_a_root_that_refuses_admission(
    ...
    # Both 2 GiB phases must fit for the gate to admit the promotion (#745);
    # the root refusal is then what defers it.
    queue = _fixture(tmp_path, ram_capacity_gib=4, landed=1)
