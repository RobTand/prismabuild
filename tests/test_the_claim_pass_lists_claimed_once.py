...
taken once per pass, after the first lock acquisition, as a hint.  A claim is
decided on a fresh listing: just before its rename, under the key's lock,
``claimed/`` is listed again and the key refused on a claim record or on
either finish mark (a tombstone or a late-finish record) filed since, exactly
as the per-candidate listing refused it.  That is one listing per claim, not
one per candidate.

A snapshot read under one claimant's placement and claimed under another's
still orders by aging: a record the claim can place that arrived without its
``passes/`` count has it read before the pass orders its candidates.
...
def test_a_pass_that_claims_lists_claimed_once_more_for_its_claim(
        tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def _file_after_the_pass_listing(monkeypatch: pytest.MonkeyPatch,
                                 claimed_dir: Path, name: str,
                                 body: str) -> None:
    """File ``claimed/<name>`` right after the pass's first listing of it."""
    ...
    def listdir(path=".", *args, **kwargs):  # type: ignore[no-untyped-def]
        names = real_listdir(path, *args, **kwargs)
        if (isinstance(path, (str, os.PathLike)) and Path(path) == claimed_dir
                and not (claimed_dir / name).exists()):
            (claimed_dir / name).write_text(body)
        return names

    monkeypatch.setattr(os, "listdir", listdir)


@pytest.mark.parametrize("mark", ["tombstone", "late-finish"])
def test_a_finish_mark_filed_after_the_listing_refuses_the_key(
        tmp_path: Path, monkeypatch: pytest.MonkeyPatch, mark: str) -> None:
    """Another box claimed, finished and requeued the key after the listing.

    Its claim record is gone by the time this pass reaches the key; what is
    left in ``claimed/`` is the finish mark, beside a ready record for the
    key again.  ``sweep_finish_tombstones`` completes the predecessor's
    cleanup only while no successor is claimed, so a claim beside the mark
    would skip that cleanup.  The pass must refuse the key, exactly as a
...
def test_a_snapshot_read_under_another_placement_still_ages(
        tmp_path: Path) -> None:
    """Discovery and the claim disagree on placement; aging still decides.

    ``ready_items`` inside :func:`pool.ready_placement` skips the ``passes/``
    read of every record that placement cannot place.  If the snapshot a
    claim is handed was read under another placement than the claim's own,
    a record the claim can place arrives without its aging count, and would
    ...
    queue.passes_path(aged).write_text(json.dumps(
        {"action_key": aged, "passes": 5}) + "\n")
    with pool.ready_placement(("elsewhere",), False):
        snapshot = queue.ready_items()
    assert [item["action_key"] for item in snapshot] == [older, aged]
    claimed = queue.claim(tags=["here"], owner="worker", ready=snapshot)
    assert claimed is not None and claimed["action_key"] == aged
