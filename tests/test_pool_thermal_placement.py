"""Cross-resource placement prefers cooler alternatives without stranding work."""
import socket
import time
...
@pytest.fixture(params=[False, True], ids=['cpu-on-gpu-heavy', 'gpu-on-cpu-heavy'])
def placement(tmp_path, request):
    queue = pool.PoolQueue(tmp_path / 'queue')
    capacity = {'cpu': 4, 'gpu': 1, 'mem_gb': 4}
    tiers = {'preferred': [0, 1, 2, 3], 'fallback': []}
    gpu = request.param
    def announce(host, *, heavy=False, tags=(), stale=False):
        ...
        queue.announce(host=host, tags=tags, has_gpu=True, capacity=capacity,
                       ...
                       observed_detail={'observed_unix': time.time() - (10 if stale else 0),
                                        'load1': 3. if heavy else 0.,
                                        'gpu_power_fraction': .75 if heavy else .1,
                                        'gpu_power_sampled_unix': time.time() - (10 if stale else 0)})
    announce(socket.gethostname(), heavy=True, tags=['local'])
    announce('other')
    key = 'a' * 64
...
def test_heavy_opposite_resource_defers_to_cooler_worker(placement, monkeypatch):
    ...
    assert claim()  # preference is bounded, never a new resource veto


@pytest.mark.parametrize('reason', ['busy', 'memory', 'stale', 'incompatible'])
def test_without_viable_cooler_alternative_work_proceeds(placement, reason):
    queue, announce, claim, key = placement
    if reason == 'busy':
        announce('other', heavy=True)
    elif reason == 'memory':
...
def test_stale_local_load_does_not_defer(placement):
    queue, announce, claim, key = placement
    announce(socket.gethostname(), heavy=True, tags=['local'], stale=True)
    assert claim()
